...
基于[express-jwt](https://github.com/auth0/express-jwt)的gum-jwt-express模块，支持通过http请求校验jwt（建议校验模块放到内网）。其余功能继承express-jwt.

**注意： 因为修改后的中间件含有异步请求，因此使用时需要先封装一下（参考下面的第一个例子）**
...
```javascript
var jwt = require('gum-jwt-express');

const jwtValidator = async (req, res, next) => (await jwt({ secret: 'shhhhhhared-secret' }))(req, res, next);
...
const jwtValidator = async (req, res, next) => (await jwt({ authServerUrl: 'http://xmc-live-auth-service/v1/secret' }))(req, res, next);
